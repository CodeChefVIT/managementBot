        ...
        if(len(rows)):
            ...
            for row in rows:
                if(row[0]==message.channel.name and row[1]==message.author.name):
                    ...
                    cur.execute("UPDATE DISCORDBOT set MSGCNT = %s and ROLES = %s where CHANNEL = %s and USERNAME = %s and SERVER = %s", (int(int(row[2])+1), str("!.#$%".join(role_str)), str(message.channel.name), str(message.author.name),str(message.guild)))
                    print(message.author.name,message.channel.name,int(row[2])+1,date.today())
                    conn.commit()
